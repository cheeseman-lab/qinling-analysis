#!/bin/bash
# Run merge step with Google Batch executor
#
# Prerequisites:
# 1. Run 5.configure_merge_params.ipynb to update config files
# 2. Upload config/ to GCS: ../gcs_utils/upload_data.sh config/ gs://bucket/screen/config/
# 3. Complete SBS and phenotype steps first (4.run_sbs_phenotype_batch.sh)
...
log_file="$SCRIPT_DIR/batch/merge-${start_time_formatted}.log"
