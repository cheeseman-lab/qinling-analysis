#!/bin/bash
# Run aggregate step with Google Batch executor
#
# Prerequisites:
# 1. Run 7.configure_aggregate_params.ipynb to update config files
# 2. Upload config/ to GCS: ../gcs_utils/upload_data.sh config/ gs://bucket/screen/config/
# 3. Complete merge step first (6.run_merge_batch.sh)
...
log_file="$SCRIPT_DIR/batch/aggregate-${start_time_formatted}.log"
